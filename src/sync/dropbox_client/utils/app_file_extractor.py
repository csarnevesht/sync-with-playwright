...
class AppFileExtractor:
    ...
    def _extract_info_from_file(self, file: FileMetadata) -> Dict[str, Any]:
        """Extract information from a file using Ollama."""
        try:
            # Download the file
            _, response = self.dbx.files_download(file.path_display)
            ...
            # Extract text and process with Ollama
            ...
            # Process with Ollama
            ollama_results = self._process_file(file)
            if not ollama_results:
                logger.warning(f"No results from Ollama for {file.name}")
                return {}
